...
namespace numeric_helper {
    ...
    NumericVariable::NumericVariable(int id_, int id_at, double lb_, double ub_): id_var(id_), id_abstract_task(id_at), lower_bound(lb_), upper_bound(ub_) {
    ...
    void NumericTaskProxy::build_numeric_variables(const TaskProxy &task_proxy){
        ...
        for (size_t num_id = 0; num_id < num_variables.size(); ++num_id){
            if(num_variables[num_id].get_var_type() == regular){
                NumericVariable nv(numeric_variables.size(),num_id,num_variables[num_id].get_initial_state_value(),num_variables[num_id].get_initial_state_value());
    ...
    void NumericTaskProxy::build_artificial_variables(const TaskProxy &task_proxy){
        ...
        for (size_t ax_id = 0; ax_id < assignment_axioms.size(); ++ax_id){
            int affected_variable = assignment_axioms[ax_id].get_assignment_variable().get_id();
            ...
            switch(assignment_axioms[ax_id].get_arithmetic_operator_type()) {
                case sum:
                    for (size_t num_id = 0; num_id < n_numeric_variables; ++num_id){
                        artificial_variables[affected_variable].coefficients[num_id] = artificial_variables[lhs].coefficients[num_id] + artificial_variables[rhs].coefficients[num_id];
                    }
                    artificial_variables[affected_variable].constant = artificial_variables[lhs].constant + artificial_variables[rhs].constant;
                    break;
                case diff:
                    for (size_t num_id = 0; num_id < n_numeric_variables; ++num_id){
                        artificial_variables[affected_variable].coefficients[num_id] = artificial_variables[lhs].coefficients[num_id] - artificial_variables[rhs].coefficients[num_id];
                    ...
                    break;
                case mult:
                    assert((num_variables[lhs].get_var_type() != constant) || (num_variables[rhs].get_var_type() != constant));
                    if (num_variables[lhs].get_var_type() == constant){
                        for (size_t num_id = 0; num_id < n_numeric_variables; ++num_id){
                            artificial_variables[affected_variable].coefficients[num_id] = artificial_variables[lhs].constant * artificial_variables[rhs].coefficients[num_id];
                        }
                        artificial_variables[affected_variable].constant = artificial_variables[lhs].constant * artificial_variables[rhs].constant;
                    }else{
                        for (size_t num_id = 0; num_id < n_numeric_variables; ++num_id){
                            artificial_variables[affected_variable].coefficients[num_id] = artificial_variables[lhs].coefficients[num_id] * artificial_variables[rhs].constant;
                        }
                        artificial_variables[affected_variable].constant = artificial_variables[lhs].constant * artificial_variables[rhs].constant;
                    }
                    break;
    ...
    void NumericTaskProxy::build_numeric_conditions(const TaskProxy &task_proxy){
        ...
        for (size_t num_id = 0; num_id < axioms.size(); ++num_id){
            ...
            switch(comp) { //    lt = 0, le = 1, eq = 2, ge = 3, gt = 4, ue = 5
                case lt:
                {
                    LinearNumericCondition lnc = artificial_variables[right] - artificial_variables[left];
                ...
                case eq:
                {
                    numeric_conditions.push_back(artificial_variables[left] - artificial_variables[right]);
                    numeric_conditions_id[num_id].push_back(n_conditions);
                    n_conditions++;
                    numeric_conditions.push_back(artificial_variables[right] - artificial_variables[left]);
                    numeric_conditions_id[num_id].push_back(n_conditions);
                    n_conditions++;
                    break;
                }
                case ge:
                {
                    LinearNumericCondition lnc = artificial_variables[left] - artificial_variables[right];
                    numeric_conditions.push_back(lnc);
                    numeric_conditions_id[num_id].push_back(n_conditions);
                    n_conditions++;
                    break;
    ...
    void NumericTaskProxy::build_actions(const TaskProxy &task){
        ...
        for (size_t op_id = 0; op_id < ops.size(); ++op_id) {
            ...
            //cout << op.get_id() << " " << op.get_name() <<  " " << op.get_preconditions().size() <<  " " << op.get_effects().size() << " " <<  op.get_ass_effects().size() << endl;
            for (FactProxy condition : op.get_preconditions()) {
                ...
                //cout << "\tpre " << pre_var_id << " " << condition.get_value() << endl;//effect.get_name() << endl;
            ...
            for (EffectProxy effect_proxy : op.get_effects()) {
                ...
                //cout << "\teff " << var << " " << post << endl;//effect.get_name() << endl;
                assert(post != -1);
                //assert(pre != post);
            ...
            set<int> intersect;
            set_intersection(pre_it,pre_end,del_it,del_end,inserter(intersect,intersect.begin()));
            actions[op_id].pre_del_list = intersect;
            //cout << "intersections " << op_id << " " << intersect.size() << " " << actions[op_id].pre_list.size() << " " << actions[op_id].del_list.size() << endl;
            // numeric effects
            ...
            for (size_t eff_id = 0; eff_id < effs.size(); ++eff_id){
                ...
                //cout << "\t" << eff.op_index << " " << eff.eff_index << " " << eff.is_axiom << endl;
        ...
        for (size_t op_id = 0; op_id < ops.size(); ++op_id) {
            ...
            for (size_t op_a_id = 0; op_a_id < ops.size(); ++op_a_id) {
                ...
                for (EffectProxy effect_proxy : op_a.get_effects()) {
                    ...
                    if (op_a.get_name().find("forget")==0 && ops[op_id].get_name().find("forget")==0) continue;
                    if ( (precondition[var]!=post && precondition[var]!=-1) || (postcondition[var]!=post) ) {
                        mutex_actions[op_id].insert(op_a_id);
                        //if (op_a.get_name().find("forget")==0 && ops[op_id].get_name().find("forget")==0)
                        //    cout << op_a.get_name() << " " << ops[op_id].get_name() << " are mutex" << endl;
                    }
                }
            }
        }
        ...
        for (size_t nc_id = 0; nc_id < get_n_conditions(); ++nc_id){
            for (size_t op_id = 0; op_id < task.get_operators().size(); ++op_id){
                ...
                for (size_t v = 0; v < n_numeric_variables; ++v){
                    cumulative_effect+=(nc.coefficients[v]*get_action_eff_list(op_id)[v]);
                }
        ...
        unordered_map<int,list<int>> axiom_table;
        {
            ...
            for (OperatorProxy ax : axioms){
                for (FactProxy pre : ax.get_preconditions()){
                    ...
                    if (ax.get_effects().size() == 0)
                        continue;
                    axiom_table[ax.get_effects()[0].get_fact().get_variable().get_id()].push_back(pre.get_variable().get_id());
                }
            }
        }
...
//                        cout << "\tafter " << numeric_conditions.size() << " " << numeric_conditions_id.size() << " " << n_conditions << endl;
    ...
    void NumericTaskProxy::calculates_bounds_numeric_variables(){
        // check if actions with effects on a real variable x : x + c have a precondition - w*x > b, with c > 0, than upperbound is max(current state, b+c)
        for (size_t num_id = 0; num_id < n_numeric_variables; ++num_id){
            // get all actions
            ...
            for (size_t op_id = 0; op_id < n_actions; ++op_id){
                ...
                for (size_t c_id : actions[op_id].num_list) {
                    ...
                    for (int nc_id : numeric_conditions_id[c_id]){
                        ...
                        // check if it's a condition of the form x + c : all other coefficients are 0
                        if (!numeric_conditions[nc_id].simple_condition(num_id)) continue;
                        // we have a condition of the simple type, we calculate the constraints
                        if (w*eff > 0) continue;//
                ...
                if (bound_for_this_action) {
                    ...
                    // bound not found, set to infinity and interrupt cycle (cannot because we might find a lower/upper bound)
                    if (eff > 0){
                        numeric_variables[num_id].upper_bound = 9999999;
                        //cout << "upper bound of " << num_id << " cannot be calculated" << endl;
                    }else{
                        numeric_variables[num_id].lower_bound = -9999999;
                        //cout << "lower bound of " << num_id << " cannot be calculated" << endl;
                    }
                    //break;
                }
            }
            //cout << "numeric variable " << num_id << " [" << numeric_variables[num_id].lower_bound
    ...
    void NumericTaskProxy::calculates_small_m_and_epsilons(){
        ...
        for (size_t i = 0; i < n_conditions; ++i){
            ...
            for (size_t op_id = 0; op_id < get_n_actions(); ++op_id){
                ...
                for (size_t n_id = 0; n_id < get_n_numeric_variables(); ++n_id){
                    local_epsilon+= lnc.coefficients[n_id]*get_action_eff_list(op_id)[n_id];
                }
